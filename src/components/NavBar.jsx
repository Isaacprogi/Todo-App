...
const NavBar = ({ nav, setNav, setSwitchList, search,handleFilters}) => {
    ...
    return <div className="w-full grid grid-rows-2 shadow-sm shadow-gray-500 h-full bg-[#373152] z-[999]    p-2 flex ">
        <span onClick={handleLogout} className='absolute top-7 flex items-center justify-center  right-[7.3rem] lg:right-[4.5rem] w-[5rem] hover:bg-gray-300 cursor-pointer p-1 rounded-md right-20 bg-gray-100'>Logout</span>
        <div className="top  flex justify-between items-center">
            ...
            <div className="icons text-lg sm:text-[1.3rem] bg-gray-900 py-1 rounded-md overflow-hidden flex flex-row items-center lg:w-full lg:justify-between  ">
                <span onClick={() => setSwitchList(true)} className='mr-1 text-white text-3xl cursor-pointer text-2xl lg:text-3xl  p-2 rounded-full focus:bg-blue-100 lg:hover:bg-gray-800 '><FcTodoList /></span>
                <span onClick={() => setSwitchList(false)} className='mr-4 text-white text-3xl cursor-pointer text-2xl lg:text-3xl   p-2 rounded-full focus:bg-blue-100 lg:hover:bg-gray-800  '><BsCalendar4Event /></span>
            </div>
        </div>
